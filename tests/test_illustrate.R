...
devtools::load_all()
source("test_main_filter.R")

df <- all_data
hold <- df
ACMG_threshold_to_plot <- 2

rm(list=setdiff(ls(), c("ACMG_threshold_to_plot", "df", "df", "geneset_MCL_ID", "output_ID")))
...
# Identdfify canonical transcript ----
# Data sources ----
...
zip_path_gff <- "../inst/extdata/uniprot/uniprot-filtered-organism%3A%22Homo+sapiens+%28Human%29+%5B9606%5D%22.gff.zip"
zip_path_csv <- "../inst/extdata/uniprot/uniprot-filtered-organism%3A%22Homo+sapiens+%28Human%29+%5B9606%5D%22.tab.zip"

temp_dir <- tempdir()

gff_file <- unzip(zip_path_gff, exdir = temp_dir)[1]
df_uniprot <- readGFF(gff_file)

csv_file <- unzip(zip_path_csv, exdir = temp_dir)[1]
df_uniprot_meta <- fread(csv_file, sep = "\t")

# Clean up unzipped files
file.remove(c(gff_file, csv_file))
rm(gff_file, csv_file)
...
df <- df_top
# recover original df from `hold` if required

# fill uniprot seqid ----
# get first entry for Uniprot_acc ID (delim = &)
df$seqid <- sapply(strsplit(as.character(df$Uniprot_acc), "&"), "[", 1)
...
df |> select(SYMBOL, seqid)
df_missing <- df |> filter(is.na(seqid)) |> dplyr::select(SYMBOL) |> unique()
df_missing
...
# 1. Identify SYMBOLs present in both df_uniprot_meta_tidy_short and df
df_uniprot_meta_tidy_short <- df_uniprot_meta_tidy |>
  dplyr::select(SYMBOL, seqid)

common_SYMBOLs <- intersect(df_uniprot_meta_tidy_short$SYMBOL, df$SYMBOL)
...
df$seqid_old <- df$seqid

df <- left_join(df, grouped_df_max, by = "SYMBOL", suffix = c("", ".y")) |>
...
# subset
unique_uniprot <- unique(df$seqid)
filtered_dt_uniprot <- dt_uniprot[dt_uniprot$seqid %in% unique_uniprot, ]

# get variant protein position ----
df_position <- df |> dplyr::select(SYMBOL, Protein_position, CDS_position, rownames, seqid, ACMG_score) |> unique()

df_position$ACMG_score <- as.numeric(df_position$ACMG_score)
df_position$Protein_position <- as.numeric(df_position$Protein_position)
df_position$Protein_position
# Remove NA - maybe use NA to detect non-coding variants like splice site
df_position <- df_position[!is.na(df_position$Protein_position), ]

# add SYMBOL to uniprot ----
df_seqid_SYMBOL <- df |> dplyr::select(SYMBOL, seqid) |> unique()
filtered_dt_uniprot <- merge(df_seqid_SYMBOL, filtered_dt_uniprot )
  ...
  plot_list <-
    lapply(split(filtered_dt_uniprot, filtered_dt_uniprot$seqid, drop = TRUE),
           function(x_dt_uniprot) {
             ...
             current_protein_positions <- df_position |>
             ...
             # Filter seqid present in df_position$seqid
             current_seqid <- x_dt_uniprot$seqid[x_dt_uniprot$seqid %in% df_position$seqid] |> unique()
...
saveRDS(df, paste0("../../data/singlecase/acmguru_gene_illustrate_df", paste(geneset_MCL_ID, collapse="_"), ".Rds"))

saveRDS(df_position, paste0("../../data/singlecase/acmguru_gene_illustrate_df_position", paste(geneset_MCL_ID, collapse="_"), ".Rds"))
